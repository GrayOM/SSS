...
## 프로젝트 개요
로컬 실행형 업로드/전처리 웹 애플리케이션입니다. 사용자가 ZIP을 업로드하면 서버가 안전하게 압축을 해제하고, 분석 가능한 파일만 선별해 JSON 목록으로 반환합니다.
...
2. 보안 검증 후 압축 해제
3. 파일 필터링 정책 적용
4. 분석 대상 목록(`path`, `extension`, `size`, `reason`, `reason_code`, `priority`, `content_hash`) 반환
