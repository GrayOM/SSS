...
def build_console_poc_analysis_prompt(files: list[FileContent]) -> str:
    ...
    for f in files[:20]:
        for idx, snip in enumerate(_keyword_snippets(f.content), 1):
            ...
            escaped_snippet = html.escape(snip['snippet'], quote=False)
            sections.append(
...
def build_candidate_analysis_prompt(files: list[FileContent], candidates: list[ApiCallCandidate], project_understanding: ProjectUnderstandingResult | None = None) -> str:
    ...
    return (
        ...
        "CRITICAL: Every finding MUST include a concrete source->sink data-flow chain with line numbers.\n"
        "  - source: the specific expression where attacker-controlled input enters (e.g. 'location.hash line 12', 'sessionStorage.getItem line 5').\n"
        "  - sink: the specific dangerous operation reached (e.g. 'innerHTML assignment line 18', 'axios.post /api/pay line 30').\n"
        "  - data_flow_steps: ordered list of intermediate steps from source to sink.\n"
        "DROP any finding that cannot show a concrete source expression with a line number AND a concrete sink with a line number.\n"
        ...
        "In evidence include: source_path, start_line, end_line, snippet, reason, source_expr (attacker-controlled input expression), sink_expr (dangerous op expression), data_flow_steps (ordered list).\n"
